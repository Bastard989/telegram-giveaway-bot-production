...
async def save_giveaway(user_id: int, state: FSMContext):
    ...
    give_reserve_winners_count = state_data.get('give_reserve_winners_count', 0)

    over_date = timezone_info.localize(datetime.datetime(
        year=give_date.year,
    ...
    await GiveAway().create_give(
        ...
        reserve_winners_count=give_reserve_winners_count,
        participation_mode='button' if give_type == 'button' else 'comments',
    )
