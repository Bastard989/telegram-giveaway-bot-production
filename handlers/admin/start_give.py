...
from database import GiveAway, GiveawayCondition, TelegramChannel, GiveAwayStatistic
...
async def start_give(jam: types.CallbackQuery, state: FSMContext):
    ...
    publish_channel = await channel_model.get_publish_channel(
        give_callback_value=give_callback_value
    )
    conditions = await GiveawayCondition().get_conditions(
        giveaway_callback_value=give_callback_value
    )

    if not publish_channel:
        await jam.answer('Сначала выберите канал публикации')
        return

    if not conditions:
        await jam.answer('Добавьте хотя бы одно условие подписки')
        return
    ...
    for give in give_data:
        ...
        if give["type"] == 'comments':
            if not publish_channel.get('group_id'):
                await jam.answer('Для розыгрыша по комментариям подключите группу обсуждения к каналу публикации')
                return

            message_text += f'\n\nДля участия напишите в комментариях: {text_for_participation_in_comments_giveaways}'
    ...
    await TelegramChannel().filter(
        give_callback_value=give_callback_value,
        role="publish",
    ).update(post_id=message_id)
    await give_model.set_publish_post(
        callback_value=give_callback_value,
        post_id=message_id,
        post_link=post_link,
    )
