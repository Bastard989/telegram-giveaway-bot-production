...
class GiveAway(Model):
    ...
    publish_channel_id = fields.BigIntField(null=True)
    publish_channel_name = fields.TextField(null=True)
    publish_post_id = fields.BigIntField(null=True)
    post_link = fields.TextField(null=True)
    participation_mode = fields.TextField(default="button")
    reserve_winners_count = fields.IntField(default=0)
    ...
    async def create_give(
        ...
        await self.create(
            ...
            reserve_winners_count=reserve_winners_count,
    ...
    async def set_publish_channel(
        ...
        await self.filter(callback_value=callback_value).update(
            publish_channel_id=channel_id,
            publish_channel_name=channel_name
        )


    async def set_publish_post(
        self,
        callback_value: str,
        post_id: int,
        post_link: str,
        ...
        await self.filter(callback_value=callback_value).update(
            publish_post_id=post_id,
            post_link=post_link
        )


    async def clear_publish_channel(self, callback_value: str):
        await self.filter(callback_value=callback_value).update(
